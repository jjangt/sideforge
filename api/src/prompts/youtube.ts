...
■ 동일 주제 성공 채널:
${trendingVideos.map(v => `- "${v.channelTitle}" (구독자: ${fmt(v.channelSubscribers || 0)}, URL: www.youtube.com/${v.channelHandle || ''}) 인기영상: "${v.title}" (조회수: ${fmt(v.views)})`).join('\n')}
` : '';
...
10. benchmarks: 위 성공 채널 데이터 활용. 없으면 빈 배열. URL 그대로 복사.
